...
XP_ligevægt1 <- function(lam1,lam2,a_plus,a_minus,eps,dt){
  ...
  for (i in 1:50){
    ...
    if (all.equal(x_ny,x, tolerance=10^-6)==TRUE && all.equal(y_ny,y, tolerance=10^-6)==TRUE) {
      break
    }
    x_ny <- x
    y_ny <- y
...
XP_ligevægt3 <- function(lam1,lam2,a_plus, a_minus ,eps, dt){
  ...
  for (i in 1:1000){
    ...
    if (all.equal(xA,x_ny, tolerance=10^-6)==TRUE & all.equal(yA,y_ny, tolerance=10^-6)==TRUE) {
      break
    }
