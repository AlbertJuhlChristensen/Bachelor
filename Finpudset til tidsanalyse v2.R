...
Data <- rbind(
  data.frame(skridt = tid_skridt, XP = sapply(tid_skridt, function(t) Strategi_funktion1(1.5,1.5,2,2,0,1/t)$XP[11,1]), funktion = "Optimise", gruppe="XP"),
  data.frame(skridt = tid_skridt, XP = sapply(tid_skridt, function(t) Strategi_funktion2(1.5,1.5,2,2,0,1/t)$XP[11,1]), funktion = "Manuelt", gruppe="XP"),
  data.frame(skridt = tid_skridt, XP = sapply(tid_skridt, function(t) Strategi_funktion3(1.5,1.5,2,2,0,1/t)$XP[11,1]), funktion = "Analytisk", gruppe="XP"),
  data.frame(skridt = tid_skridt, XP = sapply(tid_skridt, function(t) Strategi_funktion1(1.5,1.5,2,2,0,1/t)$mod_XP[11,1]), funktion = "Optimise", gruppe="mod_XP"),
  data.frame(skridt = tid_skridt, XP = sapply(tid_skridt, function(t) Strategi_funktion2(1.5,1.5,2,2,0,1/t)$mod_XP[11,1]), funktion = "Manuelt", gruppe="mod_XP"),
  data.frame(skridt = tid_skridt, XP = sapply(tid_skridt, function(t) Strategi_funktion3(1.5,1.5,2,2,0,1/t)$mod_XP[11,1]), funktion = "Analytisk", gruppe="mod_XP")
)
ggplot(data=Data, aes(x=skridt,y=XP, color=funktion, group=interaction(funktion,gruppe)))+
  geom_line()+geom_point() + geom_hline(data=data.frame(XP=XP_pois(1.5,1.5)),aes(yintercept=XP), linetype="dashed")+
  labs(x="Antal skridt", y="Forventet point", color="Metode")+theme_bw()+theme(text=element_text(size=14))
