...
lam1_grid <- seq(0.5, 3, by = 0.5)
lam2_grid <- seq(0.5, 3, by = 0.5)
a_plus_grid <- seq(0.5, 4, by = 0.5)
a_minus_grid <- seq(0.5, 4, by = 0.5)

kombinationer <- expand.grid(lam1 = lam1_grid, lam2 = lam2_grid, 
                             a_plus = a_plus_grid, a_minus=a_minus_grid)

# Histogram over antallet af iterationer før ligevægt opnås #
kombinationer$it <- mapply(function(lam1, lam2, a_plus, a_minus, eps, dt) {
  værdi <- XP_ligevægt2(lam1, lam2, a_plus, a_minus, eps, dt)
  return(værdi$it)
            }, 
