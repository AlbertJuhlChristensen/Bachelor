...
graf_funktion_X <- function(minutter,målforskel,lam1,lam2,a_plus,a_minus,eps1,eps2,eps3,dt) {
  ...
  graf <- ggplot(data, aes(y=forskel,x=XP, color=eps)) + geom_line() + geom_point() + 
    facet_wrap(~minut,ncol=3,labeller=labeller(minut= function(x) paste("Minut",(as.numeric(x)/2)-1)),  scales="free_x") + 
...
graf_funktion_X(c(2*1,2*46,2*89),2,1.6,1.4,2,2,-1,0,1, 1/180)
